...
__kernel void crop_resample32f(
    __global float* dst, int2 dst_sz,
	__global float* src, int2 src_sz, float norm, int4 crop_sz)
{

    const int x = get_global_id(0);
    const int y = get_global_id(1);
    const float gx = x / (float)dst_sz.s0 * (crop_sz.s2 - 1);
    const float gy = y / (float)dst_sz.s1 * (crop_sz.s3 - 1);
    const int gxi = (int)gx + crop_sz.s0;
    const int gyi = (int)gy + crop_sz.s1;
    const float c00 = src[pixel_idx(src_sz.s0, gxi, gyi)];
    const float c10 = src[pixel_idx(src_sz.s0, gxi + 1, gyi)];
    const float c01 = src[pixel_idx(src_sz.s0, gxi, gyi + 1)];
    const float c11 = src[pixel_idx(src_sz.s0, gxi + 1, gyi + 1)];
    const float tx = gx + crop_sz.s0 - gxi;
    const float ty = gy + crop_sz.s1 - gyi;
    dst[pixel_idx(dst_sz.s0, x, y)] =
        mix(mix(c00, c10, tx), mix(c01, c11, tx), ty)*norm;
}

__kernel void crop_resample32fc4(
    __global float4* dst, int2 dst_sz,
	__global float4* src, int2 src_sz, float norm, int4 crop_sz)
{
...
__kernel void crop_resample32fc8(
    __global float8* dst, int2 dst_sz,
	__global float8* src, int2 src_sz, float norm, int4 crop_sz)
{
